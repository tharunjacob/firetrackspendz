...
export const OnboardingGuide: React.FC<Props> = ({ hasTransactions, hasAssets, onStartUpload }) => {
  ...
  return (
    <div className="mb-8">
      <div className="card p-6 bg-gradient-to-r from-brand-50 to-purple-50 border border-brand-100">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-800">Welcome to TrackSpendZ</h2>
            <p className="text-sm text-slate-500">Get set up in 5 minutes. Here's your roadmap.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {steps.map((step) => {
            ...
            return (
              <div key={step.id} className={`flex gap-3 p-3 rounded-xl transition-colors ${done ? 'bg-green-50/80 dark:bg-green-950/30' : 'bg-white dark:bg-slate-800'}`}>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-lg ${done ? 'bg-green-100' : 'bg-slate-100'}`}>
                  {done ? '✓' : step.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-semibold ${done ? 'text-green-700 line-through' : 'text-slate-700'}`}>
                    {step.title}
                  </p>
