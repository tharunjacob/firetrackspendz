...
export const OnboardingGuide: React.FC<Props> = ({ hasTransactions, hasAssets, onStartUpload }) => {
  ...
  const [dismissed, setDismissed] = useState(() => {
    try { return localStorage.getItem('tsz_onboarding_dismissed') === 'true'; } catch { return false; }
  });

  const dismiss = () => {
    try { localStorage.setItem('tsz_onboarding_dismissed', 'true'); } catch {}
    setDismissed(true);
  };

  if (completedSteps >= 3 || dismissed) return null;

  return (
    <div className="mb-8">
      <div className="card p-6 bg-brand-50 dark:bg-slate-800 border border-brand-100 dark:border-slate-700">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-10 h-10 bg-brand-600 text-white rounded-lg flex items-center justify-center text-lg font-bold shrink-0">
