...
export const FileUploader = ({ onStartAnalysis, isProcessing, progress }: FileUploaderProps) => {
  ...
  const PasswordPrompt = ({ row }: { row: FileRow }) => row.needsPassword ? (
    <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 w-full">
      <svg className="w-4 h-4 text-amber-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
      </svg>
      <div className="flex flex-col gap-1 flex-1">
        <span className="text-xs text-amber-700 font-medium">This PDF is password-protected</span>
        <input
          type="password"
